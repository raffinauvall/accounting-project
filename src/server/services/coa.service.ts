...
export async function listCoa() { const organizationId = await requireOrganizationId(); return prisma.coaAccount.findMany({ where: { organizationId }, orderBy: { code: "asc" }, include: { children: true } }); }
...
export async function createCoa(raw: unknown, userId: string) {
  ...
  if (input.parentId) {
    const parent = await prisma.coaAccount.findFirst({ where: { id: input.parentId, organizationId } });
  ...
  const account = await prisma.coaAccount.create({ data: { ...input, organizationId, parentId: input.parentId ?? null } });
...
export async function updateCoa(id: string, raw: unknown, userId: string) {
  ...
  const input = coaSchema.parse(raw);
  if (input.statementType !== reportFor(input.accountType)) throw new Error("Tipe akun dan laporan tidak cocok");
  const current = await prisma.coaAccount.findFirst({ where: { id, organizationId }, include: { _count: { select: { entries: true, children: true } } } });
  ...
  if (input.parentId) {
    const parent = await prisma.coaAccount.findFirst({ where: { id: input.parentId, organizationId } });
    if (!parent || parent.statementType !== input.statementType || parent.accountType !== input.accountType || parent.isPostingAccount) throw new Error("Parent COA tidak cocok");
  }
  const account = await prisma.coaAccount.update({ where: { id }, data: { ...input, organizationId, parentId: input.parentId ?? null } });
...
export async function deactivateCoa(id: string, userId: string) {
  const organizationId = await requireOrganizationId();
  const account = await prisma.coaAccount.updateMany({ where: { id, organizationId }, data: { isActive: false } });
  if (account.count !== 1) throw new Error("Akun tidak ditemukan");
