...
export async function listPeriods() { const organizationId = await requireOrganizationId(); return prisma.accountingPeriod.findMany({ where: { organizationId }, orderBy: [{ year: "desc" }, { month: "desc" }] }); }

export async function createPeriod(raw: unknown) {
  const organizationId = await requireOrganizationId();
  const { month, year } = periodSchema.parse(raw);
  return prisma.accountingPeriod.create({ data: { organizationId, month, year, startDate: new Date(year, month - 1, 1), endDate: new Date(year, month, 0) } });
}

export async function closePeriod(id: string, userId: string) {
  const organizationId = await requireOrganizationId();
  const period = await prisma.accountingPeriod.updateMany({ where: { id, organizationId, status: PeriodStatus.OPEN }, data: { status: PeriodStatus.CLOSED } });
...
export async function deletePeriod(id: string) {
  const organizationId = await requireOrganizationId();
  const period = await prisma.accountingPeriod.findFirst({ where: { id, organizationId }, include: { _count: { select: { entries: true } } } });
