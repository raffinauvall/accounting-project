...
export default async function EntriesPage() {
  ...
  const entries = live?.entries.map((entry) => ({ id: entry.id, code: entry.coaAccount.code, name: entry.coaAccount.name, amount: entry.amount.toString() })) ?? demoAccounts.filter((account) => account.amount !== "0").map((account) => ({ id: account.id, code: account.code, name: account.name, amount: account.amount }));
  return <AppShell title="Input saldo"><div className="space-y-6"><div><div className="label text-[#138b82]">Data keuangan</div><h2 className="mt-2 text-3xl font-bold tracking-tight">Input saldo akun</h2><p className="mt-2 text-sm text-[#6b7785]">Pilih periode dan akun, lalu masukkan saldo akhirnya.</p></div><EntryForm accounts={accounts} periods={periods} entries={entries} isDemo={!live} /></div></AppShell>;
}
