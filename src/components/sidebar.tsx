...
export function Sidebar() {
  ...
  return <aside className="sticky top-0 hidden h-screen w-[248px] shrink-0 border-r border-[var(--border)] bg-[var(--card)] lg:block"><div className="flex h-20 items-center gap-3 border-b border-[var(--border)] px-6"><div className="grid h-9 w-9 overflow-hidden rounded-lg bg-[var(--primary)]"><Image src={logo} alt="Logo PT. APST" width={36} height={36} className="h-full w-full object-cover" /></div><div><div className="text-sm font-semibold tracking-tight">PT. APST</div><div className="mt-0.5 text-[10px] uppercase tracking-[.14em] text-[var(--muted-foreground)]">Ruang kerja keuangan</div></div></div><nav className="space-y-1 px-3 py-6">{navItems.map(([label, href, Icon]) => { const active = pathname === href || pathname.startsWith(`${href}/`); return <Link key={href} href={href} aria-current={active ? "page" : undefined} className={`flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm font-medium transition ${active ? "bg-[var(--muted)] text-[var(--foreground)]" : "text-[var(--muted-foreground)] hover:bg-[var(--muted)] hover:text-[var(--foreground)]"}`}><Icon size={17} strokeWidth={1.8} />{label}</Link>; })}</nav><div className="absolute bottom-6 px-6 text-xs text-[var(--muted-foreground)]">v0.1 · Mode lokal</div></aside>;
}
