...
export function ClearJournalButton() {
  return <ConfirmForm action={clearJournalDataAction} message="Semua transaksi jurnal akan dihapus. Data ini tidak dapat dikembalikan." title="Bersihkan jurnal?" confirmLabel="Hapus semua" destructive><button type="submit" className="flex h-10 items-center justify-center gap-2 rounded-md border border-[var(--destructive)] px-4 text-sm font-bold text-[var(--destructive)] hover:bg-[var(--destructive-soft)]"><Trash2 size={16} />Bersihkan jurnal</button></ConfirmForm>;
}
